...
import { AlertTriangle, CheckCircle2, ClipboardCheck, ExternalLink, Send } from 'lucide-react'
import { Button } from '../../components/common/Button'
import { EmptyState } from '../../components/common/EmptyState'
import { LoadingState } from '../../components/common/LoadingState'
import { useGetEvaluatorCheckpointSession, useGetEvaluatorCheckpointSessions, useSubmitCheckpointResult } from '../../hooks/useCheckpoint'
import type { CheckpointParticipant, CheckpointResultValue, CheckpointSession } from '../../types/checkpoint'
import { getFriendlyApiErrorMessage, isConflictError } from '../../utils/errorMessage'
import { formatDateTime } from '../../utils/formatters'

export function EvaluatorCheckpointPage() {
  const sessionsQuery = useGetEvaluatorCheckpointSessions()
  ...
  return (
    <section className="lms-page-shell">
      <div className="lms-page-hero">
        <div className="lms-page-hero-inner">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h1 className="lms-section-title">Checkpoint Reviews</h1>
              <p className="lms-section-description">Submit PASS or NOT_PASS for each assigned checkpoint participant.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {sessionsQuery.isLoading ? (
        ...
        <EmptyState message="No checkpoint sessions" description="No checkpoint rooms have been assigned to you yet." />
      ) : (
        <div className="grid gap-5">
          {sessions.map((session) => (
            <CheckpointSessionCard key={session.id} session={session} showDetailLink />
          ))}
        </div>
      )}
    </section>
...
export function EvaluatorCheckpointDetailPage() {
  const { sessionId } = useParams<{ sessionId: string }>()
  const id = Number(sessionId)
  const sessionQuery = useGetEvaluatorCheckpointSession(id, Number.isInteger(id) && id > 0)
        ...
        <div className="lms-page-hero-inner">
          <div>
            <Button asChild variant="ghost" className="mb-2 h-auto p-0 text-sm text-muted-foreground hover:bg-transparent hover:text-foreground">
              <Link to="/evaluator">Back to checkpoints</Link>
            </Button>
            <h1 className="lms-section-title">Checkpoint Session Detail</h1>
            <p className="lms-section-description">Review participants in this assigned checkpoint room.</p>
          </div>
        </div>
      </div>

      {sessionQuery.isLoading ? (
        <LoadingState message="Loading checkpoint session..." />
      ) : sessionQuery.isError ? (
        <div className="lms-alert-error">{getFriendlyApiErrorMessage(sessionQuery.error, 'Failed to load checkpoint session')}</div>
      ) : sessionQuery.data ? (
        <CheckpointSessionCard session={sessionQuery.data} />
      ) : null}
    </section>
  )
}

function CheckpointSessionCard({ session, showDetailLink = false }: { session: CheckpointSession; showDetailLink?: boolean }) {
      ...
      {participants.length === 0 ? (
        ...
        <div className="grid gap-3 p-5">
          {participants.map((participant) => (
            <CheckpointParticipantReview key={participant.id} participant={participant} />
          ))}
        </div>
      )}
    </article>
...
function CheckpointParticipantReview({ participant }: { participant: CheckpointParticipant }) {
  const submitResultMutation = useSubmitCheckpointResult()
  const [result, setResult] = useState<CheckpointResultValue>('PASS')
  ...
  const submitResult = async () => {
    ...
    try {
      ...
      setMessage(`Checkpoint result saved as ${response.result || result}.`)
      setComment('')
    } catch (err) {
      const fallback = isConflictError(err)
        ? 'This participant already has a result or is no longer eligible.'
        : 'Failed to submit checkpoint result'
    ...
    <div className="rounded-lg border border-border bg-white p-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Enrollment #{participant.enrollmentId ?? '-'} - added {formatDateTime(participant.addedAt)}</p>
        </div>
        {participant.result && (
      ...
      {alreadyReviewed ? (
        <div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-900">
          <p className="font-extrabold">Result submitted</p>
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-[160px_1fr_auto] lg:items-start">
          <select value={result} onChange={(event) => setResult(event.target.value as CheckpointResultValue)} className="lms-input" data-testid={`checkpoint-result-${participant.id}`}>
          ...
          <textarea
            ...
            data-testid={`checkpoint-comment-${participant.id}`}
          />
          <Button type="button" disabled={submitResultMutation.isPending} onClick={() => void submitResult()} data-testid={`submit-checkpoint-result-${participant.id}`}>
