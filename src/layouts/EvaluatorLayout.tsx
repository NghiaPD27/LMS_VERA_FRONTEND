...
export function EvaluatorLayout() {
  ...
  const userInitials = user?.username ? user.username.slice(0, 2).toUpperCase() : 'EV'

  return (
    <div className="vera-workspace-bg flex min-h-screen bg-[hsl(220_14%_7%)] text-foreground">
      <NoIndexSeo title="Vera Language | Evaluator workspace" />
        ...
        <div className="shrink-0 border-t border-border/80 p-3.5">
          <div className="mb-3 flex items-center gap-3 rounded-lg border border-border/60 bg-slate-900/60 p-2.5">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs font-bold text-white">{user?.username || 'Evaluator'}</p>
              <p className="text-[10px] capitalize text-muted-foreground">{user?.role || 'evaluator'}</p>
            </div>
          </div>
          <Button
            variant="outline"
